Public Class Archivos
    ...
    Public Shared Sub GuardarAlumnos(alumnos As List(Of Alumno), rutaArchivo As String)
        Using writer As New System.IO.StreamWriter(rutaArchivo)
    ...
    Public Shared Function ImportarAlumnos(rutaArchivo As String) As List(Of Alumno)
        Dim alumnos As New List(Of Alumno)()
        If System.IO.File.Exists(rutaArchivo) Then
            Using reader As New System.IO.StreamReader(rutaArchivo)
                While Not reader.EndOfStream
                    Dim linea As String = reader.ReadLine()
                    ' Aquí puedes implementar la lógica para convertir la línea de texto en un objeto Alumno
                    ' Por ejemplo, si la línea tiene el formato "ID: 1, Nombre: Juan, Apellidos: Pérez, Teléfono: 1234567890, Especialidad: Informática, Matrícula: 20210001"
                    Dim partes As String() = linea.Split(New String() {", "}, StringSplitOptions.None)
                    If partes.Length >= 6 Then
                        Dim id As Integer = Convert.ToInt32(partes(0).Replace("ID: ", ""))
                        Dim nombre As String = partes(1).Replace("Nombre: ", "")
                        ...
                        Dim telefono As String = partes(3).Replace("Teléfono: ", "")
                        Dim especialidad As String = partes(4).Replace("Especialidad: ", "")
                        Dim matricula As String = partes(5).Replace("Matrícula: ", "")
                        alumnos.Add(New Alumno(id, nombre, apellidos, telefono, especialidad, matricula))
                    End If
                End While
            End Using
        End If
